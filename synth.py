...
class QUICAbstraction():
	...
	def __init__(self, auto, table, adapter_ip, adapter_port):
		...
		self.inp_mask  = lambda x: []
		self.out_mask = lambda x: [ntz(cmm(x, 0, 'Message', 'Frames', -1, 'Message', 'StreamDataLimit'))] 
		self.ignorable_inp = [True, True]
		self.ignorable_out = [False, False]
		self.ninps = 0
		self.nouts = 1
			...
			for ig_sum in counter():
				...
				for nreg in counter():
					...
					if s.check() == z3.sat:
						...
						out_igns = igns
						m = s.model()
						file = open(save_path+"{}_{}.dot".format(ig_sum, nreg), "w+")
						...
						if c == "y":
							...
							v, _, _, _ = enrich(map(lambda x: (x[0], x[2]), new_traces), len(edges), nreg, abstraction.ninps, abstraction.nouts, ignorable_inp, ignorable_out, ig_sum)
							s = z3.Solver() 
							s.add(z3.And(*gtno))
							s.add(machine)
							s.add(v)
...
synth = Synthesizer("/code/config.yaml")
synth.synthesize()
